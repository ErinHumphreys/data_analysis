...
```{r}
line_plot <- line_plot + geom_line(data = mv,aes(x = date,y = pos_test_per),color = "red")+
  geom_line(data = mv,aes(x = date,y = mov_av), color = "grey",linetype = "twodash")
line_plot
```
...
```
```{r}
g1 <-  ggplot_gtable(ggplot_build(line_plot))
g2 <- ggplot_gtable(ggplot_build(bar_plot))
...
g <- gtable_add_grobs(g,ax,pp$t, length(g$widths)-1, pp$b)
...
```{r}
mv_plot <-mv_plot + line_plot +
  bar_plot %>% 
  ggplot(data = mv, mapping = aes(x = date)) +
