...
#Read raw data for Provincial Tests
```{r echo=FALSE}
...
#Write raw data
```{r}
write.csv(test_data, "data/data_raw/raw_16_provinvial_tests.csv")
```
...
```{r}
new1 <- new %>% 
  mutate(Test1 = format(Test, big.mark = ",")) # insert comma for thousands
new1
```

#Write to data_out folder
```{r}
saveRDS(new1, "data/data_out/16_provincial_tests.rds")
write.csv(new1, "data/data_out/16_provincial_tests.csv")
```

#Plot
```{r}
pt_plot <- ggplot(data = new1 ,
                  mapping = aes(x = reorder(Province, -Test), 
                                y = Test)) +
  geom_bar(stat= "identity",
           width = 0.9,
           fill = "#b54c49",
           color = "#b54c49") +
  geom_text(aes(label = Test1), 
