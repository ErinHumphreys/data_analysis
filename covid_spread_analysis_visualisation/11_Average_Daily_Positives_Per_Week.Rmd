...
#Read raw data 
```{r echo=TRUE}
media <- read.csv("data/data_raw/raw_11_average_daily_positives_per_week/case_media.csv", na.strings = "", fill = TRUE, header = TRUE, stringsAsFactors = TRUE)
media
```

#Clean data and sort them into weeks
```{r echo=TRUE}
daily_pos <- media %>% 
...
#Get comma for thousands
```{r}
daily_pos <- daily_pos %>% # for labelling purposes
  mutate(weekly_case_av1 = formatC(weekly_case_av, big.mark = ",")) # for labels
daily_pos
```

#Write to data_out folder
```{r}
saveRDS(active_cases_data, "data/data_out/11_average_daily_positives_per_week.rds")
write.csv(active_cases_data, "data/data_out/11_average_daily_postives_per_week.csv")
```
